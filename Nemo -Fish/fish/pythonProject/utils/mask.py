...
def get_train_data(sample_path, sample_img, mask_path):
    # 加载训练样本
    array_sample = scio.loadmat(sample_path)
    ...
    # 获取背景部分的RGB色值
    nemo_309_color = cv2.imread(sample_img, cv2.IMREAD_COLOR)
...
def get_mask(img_name, imgdir, sample_path, sample_img, mask_path):
    if img_name == "317.bmp":
        img_w = 170
    else:
        img_w = 174

    origin_img = cv2.imread(imgdir + "/" + img_name, cv2.IMREAD_COLOR)
    img = cv2.cvtColor(origin_img, cv2.COLOR_BGR2RGB)

    # 截断分类（方便后面腐蚀膨胀）
    img = img[:img_w, 40:220]

    bg_data, fish_data = get_train_data(sample_path, sample_img, mask_path)
    # 估计均值和方差
    ...
    out = rgb_discriminant_func(
        np.reshape(img / 255, (-1, 3)
                   ), bg_mu, bg_sigma, fish_mu, fish_sigma, w_1, w_2
    )
