...
# img_name = '311.bmp'
img_name = ["311.bmp", "313.bmp", "315.bmp", "317.bmp"]
bayes = config.bayes_root
for img in img_name:

    # 获取mask
    mask = get_mask(img, config.img_dir, sample_path=config.array_path,
                    sample_img=config.tarin_image, mask_path=config.mask_path)
    Mask = np.array(mask, dtype=np.float64)
    img_test = Image.open(config.img_dir + '/' + img)  # 需要改成测试的图像文件名
    ...
    img_rgb = np.array(img_test)
    img_gary = np.array(img_test.convert('L'))
